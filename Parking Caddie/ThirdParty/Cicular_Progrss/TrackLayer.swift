...
internal class TrackLayer: CAShapeLayer {
  ...
  fileprivate func drawTrack(_ ctx: CGContext) {
    ...
    ctx.addArc(center: CGPoint(x: centerX, y: centerY), radius: radius, startAngle: CGFloat(Math.degreesToRadians(self.setting.startAngle - Double.pi * 0.5)), endAngle: CGFloat(Math.degreesToRadians(adjustDegree - Double.pi * 0.5)), clockwise: false)
//    CGContextAddArc(ctx, centerX, centerY, radius,
//      CGFloat(Math.degreesToRadians(self.setting.startAngle - Double.pi * 0.5)),
//      CGFloat(Math.degreesToRadians(adjustDegree - Double.pi * 0.5)), 0)
    ctx.closePath();
    ctx.fillPath();
  }
}
